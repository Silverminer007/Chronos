...
public class PersonController {
    ...
    public void updateProfileImage(@PathVariable long id, @RequestBody String profileImage, Principal principal) {
        Person person = personsRepository.findById(id).filter(p -> p.equals(principal))
    ...
    public SlimPerson getPerson() {
        ...
        return new SlimPerson(principal);
    }
    ...
    public void putPerson(@RequestBody SlimPerson person) {
        Person principal = FrontendUtils.getAuthenticatedUser(this.personsRepository).orElseThrow(() -> new ResponseStatusException(HttpStatus.UNAUTHORIZED));
        this.personsRepository.save(person.getPerson(personsRepository, principal, false));
    }
