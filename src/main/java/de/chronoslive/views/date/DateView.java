package de.chronoslive.views.date;
...
import de.chronoslive.components.*;
...
import de.chronoslive.views.date.calendar.CalendarListView;
import de.chronoslive.views.date.calendar.CalendarPageView;
    ...
    private void cancelForAll(ClickEvent<MenuItem> event) {
        ...
        confirmCancelDate.addConfirmListener(confirmEvent -> {
            ...
            for (Person p : this.date.getGroup().getMembers()) {
                new MessageFormatter(p).date(this.date).person(p).feedback(this.date.getStatusFor(p)).send(Messages.DATE_CANCELLED);
            }
    ...
    private void remindAll(ClickEvent<MenuItem> event) {
        ...
        remindNow.addClickListener(e -> {
            ...
            for (Person member : date.getGroup().getMembers()) {
                result = result.and(new MessageFormatter(member).date(this.date).person(member).send(Messages.DATE_POLL));
            }
            if (result.isSuccess()) {
