...
public class OrganisationView extends VerticalLayout {
    ...
    private Dialog createMessageSetupDialog(Organisation organisation) {
        ...
        Binder<NewEMailMessageSender> emailBinder = new Binder<>();
        H1 eMailTitle = new H1("E-Mail Konfiguration");
        email.add(eMailTitle);
        TextField smtpServer = new TextField("SMTP Server");
        emailBinder.forField(smtpServer).bind(NewEMailMessageSender::getSmtpServer, NewEMailMessageSender::setSmtpServer);
        email.add(smtpServer);
        IntegerField smtpPort = new IntegerField("SMTP Port");
        emailBinder.forField(smtpPort).bind(NewEMailMessageSender::getSmtpServerPort, NewEMailMessageSender::setSmtpServerPort);
        email.add(smtpPort);
        TextField name = new TextField("Name des Absenders");
        emailBinder.forField(name).bind(NewEMailMessageSender::getSenderName, NewEMailMessageSender::setSenderName);
        email.add(name);
        TextField senderMailAddress = new TextField("E-Mail Adresse des Absenders");
        emailBinder.forField(senderMailAddress).bind(NewEMailMessageSender::getSenderEmailAddress, NewEMailMessageSender::setSenderEmailAddress);
        email.add(senderMailAddress);
        PasswordField smtpPassword = new PasswordField("Password des Absenders");
        emailBinder.forField(smtpPassword).bind(NewEMailMessageSender::getSmtpPassword, NewEMailMessageSender::setSmtpPassword);
        email.add(smtpPassword);
        NewEMailMessageSender emailSender = organisation.getEmailSender();
